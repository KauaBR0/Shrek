async function enviarScript(scriptText){
	...
	for(const line of lines){
		...
		// Aguarda um instante mínimo para o botão aparecer/ativar na tela
		await new Promise(resolve => setTimeout(resolve, 100));

		// Busca pelo novo seletor do botão de enviar
		const botaoEnviar = main.querySelector('[data-testid="wds-ic-send-filled"]')?.closest('button') 
		...
		if (botaoEnviar) {
			...
			console.error("Botão de enviar não encontrado para a linha: " + line);
		}
		
		// Intervalo entre as mensagens para evitar bloqueio do WhatsApp
...
// Para testar, cole o texto entre as crases abaixo:
enviarScript(`
SHREK
